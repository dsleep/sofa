...
void main() {
  ...
  vec3 myN = normalize(Nr.xyz);//This step is so important that the Rasteration step will use interpolattion to get N, which must be normalized.
	...
	gl_FragColor.xyz = vec3(0.6,0.6,0.6) * pow(clamp(dot(CamDir,ReflectedRay),0.1,1.0),3) + 0.2*color.xyz * clamp(dot(mylightDir, myN), -0.2,1.0) + vec3(0.1,0.05,0.0) + 0.4 * color.xyz;
  gl_FragColor.a = 0.15;//for peri-toniem
